...
// function dividendCalculation(){
   ...
   $initialPaidUpCapital=0;
    ...
    while ($shareholder = mysqli_fetch_array($shareholders)) {
        ...
        $result = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr)  from transfer where account_no = '$account'  and (value_date BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
        $soldShareArray=mysqli_fetch_assoc($result);
        $result1 = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where raccount_no = '$account' and (value_date BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
        $boughtShareArray = mysqli_fetch_array($result1);
        $boughtShare=$boughtShareArray?$boughtShareArray['sum(total_share_transfered_in_birr)']:0;
        $soldShare=$soldShareArray?$soldShareArray['sum(total_share_transfered_in_birr)']:0;
       $totalTransfered=$boughtShare- $soldShare;
        $adjestedBalance=$initialPaidUpCapital+ $totalTransfered;
        
        $resultForAdjestment = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr)  from transfer where account_no = '$account' and agreement='seller' and (transfer_date BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
        $soldShareArrayForAdjestment=mysqli_fetch_assoc($resultForAdjestment);
        $soldValueForAdjestment=$soldShareArrayForAdjestment?$soldShareArrayForAdjestment['sum(total_share_transfered_in_birr)']:0;
        $resultForAdjestment = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where raccount_no = '$account' and agreement != 'buyer' and (transfer_date BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
        $boughtShareArrayForAdjestment = mysqli_fetch_array($resultForAdjestment);
        $boughtValueForAdjestment=$boughtShareArrayForAdjestment?$boughtShareArrayForAdjestment['sum(total_share_transfered_in_birr)']:0;
        ...
        foreach($fromSold as $value) {  
            $amount=$value['total_share_transfered_in_birr']; 
            $startDate = $from;
        ...
        $paidup_utilized_sum = mysqli_query($conn,"SELECT SUM(amount) as value from paidup_utilized where start_date='$from' && end_date='$to' ") or die(mysqli_error($conn));
        $total_paidup_utilized = mysqli_fetch_array($paidup_utilized_sum); 
        $paidup_utilized = mysqli_query($conn,"SELECT * from paidup_utilized where start_date='$from' && end_date='$to' ") or die(mysqli_error($conn));
        while($paidup_utilized_row = mysqli_fetch_array($paidup_utilized)){
            $account=$paidup_utilized_row['account'];
            $ratio = $paidup_utilized_row['amount']/floatval($total_paidup_utilized['value']);
            $dividend_amt = mysqli_query($conn,"SELECT * from dividend_amount where year BETWEEN '$from' and '$to' ") or die(mysqli_error($conn));
            ...
            mysqli_query($conn,"INSERT INTO ratio_dividend (account,ratio,dividend_portion,start_date,end_date) values($account,$ratio,$dividend_portion,'$from','$to') ON DUPLICATE KEY UPDATE ratio=values(ratio),dividend_portion=values(dividend_portion)")or die(mysqli_error($conn));
        }
    // }
        ?>