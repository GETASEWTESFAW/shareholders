...
$result = mysqli_query($conn,"SELECT * FROM shareholders WHERE account_no ='$id' AND (year BETWEEN '$from' and '$to')");
...
$query4 = mysqli_query($conn,"SELECT *,sum(plamount) from pludge where account_no = '$acct' and pledged_status = 'pledged' AND year = '$year' order by id ASC") or die(mysqli_error($conn));                         
...
<div class="form-group">
    ...
    <div class="form-group">
        ...
        <input type="hidden" readonly required name="plamount" value="<?php echo $rows4['sum(plamount)']; ?>" class="form-control" placeholder="Enter ..."/>
        <input type="hidden" readonly required name="t_seller_paidup" value="<?php $t_paid_up = ($row['total_paidup_capital_inbirr'] / 500);
        $t_calc = ($t_paid_up - $rows4['sum(plamount)']); echo $t_calc; ?>" class="form-control" placeholder="Enter ..."/>
    ...
    <?php echo form_error('account_no'); ?>
     ...
     <div class="form-group">
        ...
        $name = $row['name'];
        $result = mysqli_query($conn,"SELECT * FROM shareholders where status != 'closed' AND (year BETWEEN '$from' and '$to') group by name order by account_no");
...
<input type="hidden" name="ag_year" value="<?php $month = 8; $year = date('Y');
  ...
  echo date("Y-m-d", mktime(0, 0, 0, $month - 1, 1, $year-1)); ?>">
</div>  
...
if(isset($_POST['submit'])){
  ...
  $total_share_transfered_in_birr = $seller_paidup;
  ...
  echo "remaining share".$remaining_share;
 ...
 $query = mysqli_query($conn,"INSERT INTO transfer (account_no,total_share, name,raccount_no,rname,rtotal_share,total_share_transfered,total_share_transfered_in_birr,buyyer_paidup_in_birr,seller_paidup_in_birr,transfer_date,value_date,pervalue,status_of_transfer,year,agreement,agred_to,both_seller,both_buyer,transfer_type) 
