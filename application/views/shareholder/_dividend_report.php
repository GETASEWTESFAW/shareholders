...
if($budget_result){
  ...
  //$year = date('Y');
$result_tra_tra = mysqli_query($conn,"SELECT * FROM transfer WHERE account_no = '$name' and (year BETWEEN '$from' and '$to') and agreement = 'buyer' and status_of_seller = 'closed' order by account_no");
...
//do sold and bought share have r/ship?
      ...
      $po_trans = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where account_no = '$name' and (year BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));

      while($po_row = mysqli_fetch_array($po_trans)){

      $neg_trans = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where raccount_no = '$name' and (year BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
      ...
      <td><?php echo number_format($share_subscribed_inbirr2 + $neg_row['sum(total_share_transfered_in_birr)'] - $po_row['sum(total_share_transfered_in_birr)'] ,2); } } } ?></td>
      ...
      <td><?php 
        ...
        $po_trans = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where raccount_no = '$name' and (value_date BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
                        
        while($po_row = mysqli_fetch_array($po_trans)){
        
          $neg_trans = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where account_no = '$name' and (year BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
      ...
      while($neg_row = mysqli_fetch_array($neg_trans)){
        ...
        if($po_row['sum(total_share_transfered_in_birr)'] == '0' && $neg_row['sum(total_share_transfered_in_birr)'] =='0'){
            
            echo number_format($share_subscribed_inbirr2,2);

          } elseif($po_row['sum(total_share_transfered_in_birr)'] > '0' && $neg_row['sum(total_share_transfered_in_birr)'] =='0'){
            
            echo number_format($share_subscribed_inbirr2,2);

          } elseif($neg_row['sum(total_share_transfered_in_birr)'] > '0' && $share_subscribed_inbirr2 > '0' ) {

            //echo "share subscribed 2".$neg_row['sum(total_share_transfered_in_birr)'];
        
         echo number_format($share_subscribed_inbirr2 - $neg_row['sum(total_share_transfered_in_birr)'] ,2); 
        } 
         else {
      ...
      <td><?php echo number_format($cap['sum(capitalized_in_birr)'],2); ?></td>
            ...
            $result = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where account_no = '$name' and (year BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
            ...
            $result1 = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where raccount_no = '$name' and (year BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
            ...
            <?php echo number_format($share_subscribed_inbirr2 - $row['sum(total_share_transfered_in_birr)'] + $fetch['sum(total_share_transfered_in_birr)'] + $cap['sum(capitalized_in_birr)'],2);

              /*echo "Share Subsc".$share_subscribed_inbirr2."<br>";

              echo "Transfered".$row['sum(total_share_transfered_in_birr)']."<br>";

              echo "Received".$fetch['sum(total_share_transfered_in_birr)']."<br>";
...
<tbody>
      ...
      while($cash_trans = mysqli_fetch_array($result_trans_value)) {
        ...
        //print_r($cash_trans);
      ?><br>__________<br><?php echo number_format($cash_trans['total_share_transfered_in_birr'],2); } ?>
      ...
      while($cash = mysqli_fetch_array($resultp)) {
        ...
        $cap_cash = ($cash['total_share_transfered_in_birr'] * $dif) / 365;
      ?>
    ...
    <?php
      ...
      while($neg_row = mysqli_fetch_array($neg_trans)){             
        ...
        //$share_subscribed_inbirr2 - $po_row['sum(total_share_transfered_in_birr)'];
  ...
  if($po_row['sum(total_share_transfered_in_birr)'] == '0' && $neg_row['sum(total_share_transfered_in_birr)'] =='0'){
            ...
            $adjusted_balance = $share_subscribed_inbirr2 - $neg_row['sum(total_share_transfered_in_birr)'].'<br/>';
...
//logical error
      ...
      while($cash = mysqli_fetch_array($resultp)) {
        ...
        //echo $cash['total_share_transfered_in_birr'];
    ...
    // average paid up capital 
      ...
      <?php //echo number_format(round($cap_cash,3),2);  
        ...
        $total_sum = $array_result + $share_subscribed_inbirr2 - $po_row['sum(total_share_transfered_in_birr)'];
        
        $total_paidup_value = round(array_sum($results),2) + $share_subscribed_inbirr2 - $po_row['sum(total_share_transfered_in_birr)'];
        
        //echo $po_row['sum(total_share_transfered_in_birr)'] + $neg_row['sum(total_share_transfered_in_birr)'];

        //echo number_format(round(array_sum($results),2),2);
        //echo "array sum".array_sum($results);
        //echo $share_subscribed_inbirr2 - $po_row['sum(total_share_transfered_in_birr)'] + $neg_row['sum(total_share_transfered_in_birr)'];
        //echo "po row".$po_row['sum(total_share_transfered_in_birr)'];
        ?>
    ...
    //echo sprintf('%.10f',$ratio);
      ...
      $po_trans = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where agred_to = '$name' || both_buyer = '$name' || both_seller = '$name' and (year BETWEEN '$from' and '$to') and status_of_transfer = 'authorized'") or die(mysqli_error($conn));
