...
echo '<script>alert("Value date is out of budget year!");</script>';
    ...
    capitalized_status = 'pending' and type = 'payable' and account_no = '$account_no'")or die(mysqli_error($conn));
    ...
    mysqli_query($conn,"INSERT into capitalized (name,capitalized_in_share,capitalized_in_birr,value_date,type,account_no,year,capitalized_status) values ('$name','$capitalized_share','$capitalized_in_birr','$value_date','payable','$account_no','$year','pending')") or die(mysqli_error($conn));
    ...
    echo 'alert("Payable payment more than the alloted amount is not allowed")';
    echo '</script>';
}
    ...
    echo '<script language="javascript">';
    echo 'alert("payable payment amount must be divisible by 500.")';
    echo '</script>';
} else{
