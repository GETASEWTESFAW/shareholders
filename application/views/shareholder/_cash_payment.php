...
echo '<script>alert("Value date is out of budget year!");</script>';
    ...
    echo $cap_share."capitalized in share".'<br/>';
    echo $check_allot_row."allotment ".'<br/>';

    }

    if($cap_share <= $check_allot_row){

    mysqli_query($conn,"INSERT into capitalized (name,capitalized_in_share,capitalized_in_birr,value_date,type,account_no,year,capitalized_status) values ('$name','$capitalized_share','$capitalized_in_birr','$value_date','cash','$account_no','$year','pending')") or die(mysqli_error($conn));
    ...
    echo 'alert("Paying cash more than the alloted amount is not allowed")';
    echo '</script>';
}
    ...
    echo '<script language="javascript">';
    echo 'alert("The cash payment amount must be divisible by 500.")';
    echo '</script>';
} else{
