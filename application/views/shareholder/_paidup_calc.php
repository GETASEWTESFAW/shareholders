...
$resultp1 = mysqli_query($conn,"SELECT * from transfer where status_of_transfer = 'authorized' and agred_to = '$acc' || both_buyer = '$acc' || both_seller = '$acc' and (value_date BETWEEN '$from' and '$to')") or die(mysqli_error($conn));
  ...
  while($cash1 = mysqli_fetch_array($resultp1)) {
    ...
    echo "totalshae".$cash1['total_share_transfered_in_birr'].'<br/>';

    $cap_cash = ($cash1['total_share_transfered_in_birr'] * $dif) / 365;
  ?>
...
$po_trans = mysqli_query($conn,"SELECT *,sum(total_share_transfered_in_birr) from transfer where agred_to = '$acc' || both_buyer = '$acc' || both_seller = '$acc' AND (value_date BETWEEN '$from' and '$to') AND status_of_transfer = 'authorized'") or die(mysqli_error($conn));
...
echo "po row = ".$po_row['sum(total_share_transfered_in_birr)'].'<br/>';
echo "share subscribed in birr2 = ".$share_subscribed_inbirr.'<br/>';
$adjusted = $share_subscribed_inbirr - $po_row['sum(total_share_transfered_in_birr)'];
...
$total_sum = $array_result + $share_subscribed_inbirr - $po_row['sum(total_share_transfered_in_birr)'];

/*if($cash['name'] !== '$name' && $cash['capitalized_in_birr'] == ''){

  $total_paidup_value[] = $share_subscribed_inbirr - $po_row['sum(total_share_transfered_in_birr)'];

} else {

  $total_paidup_value[] = round($array_result,2) + $share_subscribed_inbirr - $po_row['sum(total_share_transfered_in_birr)'];
...
<input type="text" value="<?php //echo $array_result; ?>"><br><br>

<input type="text" value="<?php //echo $total_paidup_value; ?>">-->

<?php //echo number_format(round(array_sum($results),2) + $share_subscribed_inbirr - $po_row['sum(total_share_transfered_in_birr)'] + $neg_row['sum(total_share_transfered_in_birr)'],2); 
