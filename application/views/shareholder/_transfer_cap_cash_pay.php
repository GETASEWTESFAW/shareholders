...
<div class="alert alert-success alert-dismissable">
    ...
    <b>Success!</b> Cash payment reversed succesfully!.
</div>
                        ...
                        <div class="col-xs-12">
                          ...
                          <?php 
                                    ...
                                    if(isset($_POST['search'])){
                                          ...
                                          $query = mysqli_query($conn,"SELECT * from capitalized where capitalized_status = 'authorized' AND name LIKE '$key%' || account_no LIKE '$key%' AND (year BETWEEN '$from' and '$to') order by id DESC") or die(mysqli_error($conn));
                                          }else{
                                            //   echo "SELECT * from capitalized where capitalized_status = 'authorized'  AND (year BETWEEN '$from' and '$to') order by id DESC";
                                        $query = mysqli_query($conn,"SELECT * from capitalized where capitalized_status = 'authorized'  AND (year BETWEEN '$from' and '$to') order by id DESC") or die(mysqli_error($conn));
                                          ...
                                          $id = $rows['id'];
                                            ...
                                            <input type="hidden" name="id" value="<?php echo $id; ?>">
                                                ...
                                                <td><?php echo $rows['type']; ?></td>
                                                 ...
                                                 <td><a href="<?php echo base_url();?>shareholder/transfer_cap_cash_final?acct=<?php echo $rows['account_no']; ?>&id=<?php echo $rows['id']; ?>&from=<?php echo $from; ?>&to=<?php echo $to; ?>">Transfer <?php echo $rows['type']; ?></a></td>
...
<?php 
        ...
        $query_cap = mysqli_query($conn,"SELECT * from capitalized where 
            capitalized_status = 'authorized' and type = 'cash' AND id = '$id[$i]'") or die(mysqli_error($conn));
        ...
        header('location:http://172.23.2.174/shareholder_dividend/shareholder/cash_rev_report?reverse=ok');
