 ...
 <div id="homepage_search">
                          ...
                          <?php 
                                    ...
                                    if(isset($_POST['search'])){
                                          ...
                                          $query="";
                                          if($key){
                                          $query = mysqli_query($conn,"SELECT * from capitalized where capitalized_status = 'authorized' AND type = 'payable' AND name LIKE '$key%' || account_no LIKE '$key%' AND (year BETWEEN '$from' and '$to') order by id DESC") or die(mysqli_error($conn));
                                        //   $rows = mysqli_fetch_array($query);
                                        //    var_dump(count($rows));
                                          }else{
                                            $query = mysqli_query($conn,"SELECT * from capitalized where capitalized_status = 'authorized' AND type = 'payable'  AND (year BETWEEN '$from' and '$to') order by id DESC") or die(mysqli_error($conn)); 
                                          }
...
<?php 
        ...
        $query_cap = mysqli_query($conn,"SELECT * from capitalized where 
            capitalized_status = 'authorized' and type = 'payable' AND id = '$id[$i]'") or die(mysqli_error($conn));
